...
graph TD
    subgraph ClientLayer ["Client & Touchpoint Layer"]
        WebPublic["Public Deal Showcase\n(/showcase, /projects/[id])"]
        WebAdmin["Executive Ops Hub\n(/admin, /financial-model)"]
        WebPortals["Stakeholder Dashboards\n(/investor, /business, /promoter, /kam-dashboard)"]
        TgMiniApp["Telegram WebApp Mini-App\n(/team-miniapp)"]
        TgBots["3 Telegram Bots\n@gro10xmanbot, @gro10xcapbot, @gro10xbizbot"]
    end

    subgraph ApiLayer ["Serverless Next.js API Layer"]
        AuthApi["Auth & PIN Resolver\n/api/telegram-auth/*"]
        WebhookApi["Universal Bot Webhook\n/api/telegram-webhook"]
        LeadApi["Attribution & Leads\n/api/submit-lead, /api/apply-cohort"]
        PushApi["Multi-Bot Notification Push\n/api/telegram-notify-*"]
        MiniAppAuth["HMAC Signature Auth\n/api/miniapp-auth/validate"]
    end

    subgraph DataLayer ["Supabase Cloud Infrastructure"]
        AuthUsers["Supabase Auth & Roles\n(user_roles, team, telegram_auth_pins)"]
        DealsDB["SPV & Investments\n(funding_projects, investments, secondary_orders)"]
        AuditsDB["Audits & POS Telemetry\n(pos_daily_sales, business_audits, cash_tickets)"]
        Storage["Storage Buckets\n(cohort-docs, asset-photos, kyc-documents)"]
    end
