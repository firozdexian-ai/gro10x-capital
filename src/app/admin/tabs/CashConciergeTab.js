...
import { Send, FileText, PlusCircle } from 'lucide-react';
import { formatCurrency } from '../../../lib/currency';

/**
 * CashConciergeTab — Upgraded Tab 6 OTC Advisory Desk & Cash Pipeline for GRO10X Admin.
 *
 * Sub-tabs:
    ...
    <div className="tab-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
        ...
        <div className="glass-card-premium" style={{ padding: '1.25rem' }}>
          ...
          <h3 style={{ fontSize: '1.35rem', fontWeight: '800', color: '#D4AF37', margin: 0 }}>
            {formatCurrency(activePipelineValue, currency)}
          </h3>
          <span style={{ fontSize: '0.72rem', color: '#D4AF37' }}>Target OTC Commitment</span>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          {cashTickets.filter(t => cashStatusFilter === 'All' || t.status === cashStatusFilter).length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem', background: 'rgba(7,10,20,0.8)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }}>
              <h3 style={{ fontSize: '1.15rem', color: '#94a3b8', margin: 0 }}>No confidential OTC consultation tickets found for this filter.</h3>
            </div>
          ) : (
