...
export async function handleKpisCommand(botToken, chatId, appUrl) {
  try {
    ...
    const [{ count: activeProjectsCount }, { count: activeInvestorsCount }, { data: investmentsData }, { data: leadsData }, { data: kycData }, { data: payData }] = await Promise.all([
...
export async function handleAlertsCommand(botToken, chatId, appUrl) {
  try {
    const [{ data: pendingKyc }, { data: pendingPayments }, { data: unworkedLeads }, { data: pendingPayouts }] = await Promise.all([
      ...
      supabase.from('payout_requests').select('id, amount_bdt, promoter_id').eq('status', 'Pending').limit(5)
    ]);
    ...
    alertText += `💳 <b>Pending Payment Clearances:</b> ${payCount}\n`;
    ...
    const keyboard = {
      inline_keyboard: [
        [
          { text: `👥 Unworked Leads (${leadCount})`, callback_data: 'cmd_leads' },
          { text: `💳 Payout Queue (${payoutCount})`, callback_data: 'cmd_payouts' }
        ],
        [
          { text: '🌐 Resolve on Web Panel', url: `${appUrl}/admin` }
        ]
      ]
    };
...
export async function handlePayoutsCommand(botToken, chatId, appUrl) {
  try {
    ...
    for (const p of payouts) {
      const promoterName = p.team?.full_name || 'Promoter';
...
// 5. /broadcast Command — Team Broadcast
export async function handleBroadcastCommand(botToken, chatId, textToBroadcast) {
  ...
  try {
    ...
    let successCount = 0;
    const msg = `📢 <b>Team Announcement</b>\n\n${textToBroadcast}\n\n<i>— Sent via GRO10X Executive Broadcast</i>`;
